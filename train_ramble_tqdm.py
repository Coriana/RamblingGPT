...
wandb_project = 'owt'
wandb_run_name = 'SkyGPT_yt2' # 'run' + str(time.time())
# data
dataset = 'whispering'
gradient_accumulation_steps = 4 # used to simulate larger batch sizes
batch_size = 50 # if gradient_accumulation_steps > 1, this is the micro-batch size
block_size = 768    
# model
n_layer = 12
n_head = 8
...
# adamw optimizer
learning_rate = 0.0001 # max learning rate
...
for i in tqdm(range(iter_num, max_iters)):
    ...
    if i % eval_interval == 0 and master_process:
        ...
        with torch.no_grad():
            start_ids = encode(add_caseifer("\n"))
            x = (torch.tensor(start_ids, dtype=torch.long, device=device)[None, ...])
            y = model.generate(x, max_new_tokens, temperature=temperature, top_k=top_k)
            print(remove_caseifer(decode(y[0].tolist()))[len(start_ids):])
